...
class SpreadController extends Controller {
            ...
            foreach ($coin_prices as $market_name => $coin_prices_market) {
                if (isset($coin_prices_market[$coin_symbol])) {
                    ...
                    if ($market_name != 'bithumb') {
                        $full_spread[$coin_symbol][$market_name . "_diff_perc"] = ($coin_prices_market[$coin_symbol] - $bithumb_price) * 100 / $bithumb_price;
                    }
                }
                // else{
    ...
    public function binance() {
        ...
        foreach ($market_coins_available as $coin_available) {
            $filtered_coins = array_filter(
                    $coin_prices['binance'][$coin_available], function ($coin_sym) use ($markets, $coin_available, $coin_prices) {
    ...
    public function bittrex() {
        ...
        foreach ($market_coins_available as $coin_available) {
            $filtered_coins = array_filter(
                    $coin_prices['bittrex'][$coin_available], function ($coin_sym) use ($markets, $coin_available, $coin_prices) {
                foreach ($markets as $market_name) {
                    if (!empty($coin_prices[$market_name][$coin_available][$coin_sym])) {
                        return true;
                    }
